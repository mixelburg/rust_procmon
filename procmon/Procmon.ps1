...
New-Module -Name Procmon -Scriptblock {
    ...
    function ConvertTo-ProcmonXML {
        ...
        End {
            ...
            Invoke-Procmon -ProcmonDir "$ProcmonDir" -ExeArgs "/Openlog `"$EventFile`" /SaveAs1 `"$OutputFile`" /SaveApplyFilter /AcceptEula" | Out-Null
            Wait-Procmon $Wait
        }

    }

    Export-ModuleMember -Function Invoke-Procmon, `
        ...
        Write-ProcmonFiltersBytesToRegistry,Clear-ProcmonFiltersRegistry,ConvertTo-ProcmonCsv,ConvertTo-ProcmonXML
}